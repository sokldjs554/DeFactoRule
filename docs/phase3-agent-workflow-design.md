...
### 4-1. trap_risk — 이 설계의 심장

Router 가 쓰는 신호 중 하나만 새로 만든다. 나머지는 산술이다.

> **trap_risk** = 이 요청에 대해 **최근접 선례를 따르면 틀릴 확률**의 dev 추정치

계산은 이렇다. 추론 시점에 알 수 있는 것만 쓴다 — 정답은 쓰지 않는다.

```
dev 에서 leave-one-out 으로 각 사례의 최근접 선례를 찾는다
   (자기 자신을 빼야 한다. 안 빼면 유사도 1.0 이 나온다)
        ↓
(유사도 구간, 선례가 가리키는 라벨) 별로
   P(선례 라벨 ≠ 실제 라벨) 을 센다
        ↓
그 표를 test 에 그대로 적용한다.  test 는 보정에 쓰지 않는다.
```

유사도 구간은 거칠게 잡는다 — `[0, 0.15) [0.15, 0.30) [0.30, 0.60) [0.60, 1.0]`.
dev 가 85건뿐이라 잘게 자르면 칸마다 한두 건이 된다.

**이 표가 왜 E5 를 그대로 옮긴 것인가**: E5 는 "조치의 앵커링이 7.1%" 라고
말했다. 그건 *정답이 조치일 때* 선례가 있을 확률이다. 추론 시점엔 정답을
모른다. 대신 **선례가 뭐라고 하는지**는 안다. 그래서 조건을 뒤집어 잡는다 —
`P(틀림 | 유사도, 선례의 라벨)`. 뒤집어도 같은 현상을 잡는다는 것은 dev 에서
확인할 수 있고, 확인하지 못하면 이 설계의 전제가 무너진다(§ 설계의 문제점 1번).
...
1. **`trap_risk` 표를 먼저 만든다** (비용 0, 코드 ~40줄). §문제점 1이 살아
   있는지 죽는지 여기서 갈린다. **이 결과를 보고하고 그다음을 정한다.**
2. 문턱 통일 (§문제점 5) → 영향받는 문서 재생성
