...
> **정정** — 위 문장의 "4/15" 는 **틀렸다.** 계산하지 않고 쓴 숫자다. 실제로는
> `1/15` = [0.012, 0.298] 로 이미 하한이 0 을 넘는다. 즉 사전 등록한 기준은
> 15건 중 1건만 맞혀도 통과하는 **약한 기준**이었다. 기준은 바꾸지 않고
> (결과를 보고 문턱을 옮기는 것이 조작이다) 그 사실을 [docs/17](17-e8-e11-agent-workflow.md)
> 에 적었다.
